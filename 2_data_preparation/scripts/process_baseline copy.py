...
# Define ONLY the baseline app we care about for this run
BASELINE_APP_FILES = ['calm']


# --- 2. Find and Load ONLY Baseline App Files ---
all_files = glob.glob(os.path.join(RAW_DATA_PATH, "*.csv"))
app_files = [f for f in all_files if any(app in f.lower() for app in BASELINE_APP_FILES)]

if not app_files:
    print("ERROR: No baseline app files ('calm') found. Please check your file names and paths.")
    exit()

print(f"Found {len(app_files)} baseline app files to process.")

# --- 3. Load, Standardize, and Combine ---
cleaned_dfs = []
for f in app_files:
    raw_df = pd.read_csv(f, low_memory=False)
    clean_df = pd.DataFrame()
    
    # Define standard columns and find raw data for them
...
df_master.drop_duplicates(subset=['user', 'date', 'text_short'], keep='first', inplace=True)
...
date_condition = (df_english['date'] >= datetime(2022, 1, 1)) | (df_english['date'].isna())
rating_condition = df_english['rating'].isin([1, 2, 3])
df_final = df_english.loc[date_condition & rating_condition] # Using .loc to avoid SettingWithCopyWarning
print(f"After final filtering for date and rating: {len(df_final)} rows remain.")

# --- 5. Finalize Schema and Save ---
df_final = df_final.rename(columns={'text': 'review_text', 'user': 'user_name'}) # Use assignment to avoid warning
...
# ** THE FINAL OUTPUT FILENAME IS CHANGED HERE **
