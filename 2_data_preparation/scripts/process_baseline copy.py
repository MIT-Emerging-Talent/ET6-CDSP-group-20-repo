...
app_files = [
    f for f in all_files if any(app in f.lower() for app in BASELINE_APP_FILES)
]

if not app_files:
    print(
        "ERROR: No baseline app files ('calm') found. Please check your file names and paths."
    )
    exit()
...
df_final = df_english.loc[
    date_condition & rating_condition
]  # Using .loc to avoid SettingWithCopyWarning
print(f"After final filtering for date and rating: {len(df_final)} rows remain.")

# --- 5. Finalize Schema and Save ---
df_final = df_final.rename(
    columns={"text": "review_text", "user": "user_name"}
)  # Use assignment to avoid warning
